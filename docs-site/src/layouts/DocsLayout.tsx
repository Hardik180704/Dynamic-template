...
const DocsLayout: React.FC<DocsLayoutProps> = ({ sidebarOpen: propSidebarOpen, setSidebarOpen: propSetSidebarOpen }) => {
  ...
  const [selectedSection, setSelectedSection] = useState(docSections.length > 0 ? docSections[0].id : "getting-started");
  const [selectedItem, setSelectedItem] = useState(docSections.length > 0 && docSections[0].items.length > 0 ? docSections[0].items[0].id : "quick-start");
  ...
  // const docSections: DocSection[] = documentationData.sections as DocSection[]; // REPLACED BY CONTEXT
  ...
  const openDocumentWindow = (section: DocSection, item: DocItem) => {
    ...
    const formattedContent = typeof item.content === 'string' ? {
      title: item.title,
    ...
    const newWindow: WindowData = {
      id: itemKey,
      title: item.title,
      icon: renderSectionIcon(section.icon || 'Database'),
      content: <DocumentationRenderer content={formattedContent as any} />,
      position: {
