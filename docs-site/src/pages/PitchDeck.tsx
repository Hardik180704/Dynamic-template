...
const PitchDeck = () => {
  const { pitchDeckUrl } = useContent();
  const isMobile = useIsMobile();
  const [numPages, setNumPages] = useState<number>(0);
  const [pageNumber, setPageNumber] = useState<number>(1);
  const [scale, setScale] = useState<number>(isMobile ? 1.0 : 0.7);
  ...
  // If no pitch deck is uploaded, show placeholder
  if (!pitchDeckUrl) {
    return (
      <div className="h-screen bg-background flex flex-col items-center justify-center p-4">
        <Home className="h-16 w-16 text-muted-foreground mb-4" />
        <h1 className="text-2xl font-bold mb-2">No Pitch Deck Uploaded</h1>
        <p className="text-muted-foreground mb-6">Please upload a PDF presentation from the Admin Dashboard.</p>
        <Link to="/admin">
